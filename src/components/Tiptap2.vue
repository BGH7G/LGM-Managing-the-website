<template>
  <div class="tiptap">
    ...
    <bubble-menu
      ...
      class="bubble-menu"
    >
      <v-btn-group variant="text" density="compact" class="bubble-menu-buttons">
        <v-btn :class="{ 'is-active': isBold }" color="primary" @click="editor.chain().focus().toggleBold().run()">
          <v-icon icon="mdi-format-bold"></v-icon>
        </v-btn>
        <v-btn :class="{ 'is-active': isItalic }" color="primary" @click="editor.chain().focus().toggleItalic().run()">
          <v-icon icon="mdi-format-italic"></v-icon>
        </v-btn>
        <v-btn :class="{ 'is-active': isSetStrike }" color="primary" @click="editor.chain().focus().toggleStrike().run()">
          <v-icon icon="mdi-format-strikethrough"></v-icon>
        </v-btn>
        <v-btn :class="{ 'is-active': isUnderline }" color="primary"  @click="editor.chain().focus().toggleUnderline().run()">
          <v-icon icon="mdi-format-underline"></v-icon>
        </v-btn>
      </v-btn-group>
    </bubble-menu>
    
    <v-dialog v-model="bubbleMenuDialogVisible" width="500">
      <v-card>
        <v-card-title>
          <span class="text-h5">添加链接</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="grey-darken-1"
            variant="text"
            @click="bubbleMenuDialogVisible = false"
          >
            取消
          </v-btn>
          <v-btn
...
const linkUrl = ref('')
const bubbleMenuDialogVisible = ref(false)
const bubbleMenuLinkUrl = ref('')
...
const CustomImage = Image.extend({
    ...
    return {
      ...
      textAlign: {
        ...
        renderHTML: attributes => {
          // 我们把对齐信息也存为 data- 属性，以便在 renderHTML 拿到
          return {
            'data-text-align': attributes.textAlign || null
          }
        },
      },
      width: {
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    // 根据 textAlign 生成 class
    const classes = ['image-wrapper']
    if (textAlign) {
      classes.push(`align-${textAlign}`)
    }
    return [
      'div',
      {
...
const showBubbleMenuDialog = () => {
  bubbleMenuLinkUrl.value = editor.value?.getAttributes('link').href || ''
  bubbleMenuDialogVisible.value = true
}
const confirmBubbleMenuLink = () => {
  if (!bubbleMenuLinkUrl.value) {
    editor.value?.chain().focus().unsetLink().run()
  } else {
    editor.value?.chain().focus().setLink({
      href: bubbleMenuLinkUrl.value,
      target: '_blank',
      rel: 'noopener noreferrer'
    }).run()
  }
  bubbleMenuDialogVisible.value = false
  bubbleMenuLinkUrl.value = ''
}
